...
def process_fb_food_dataset(file_path, model='IC'):
    edges = read_fb_food_data(file_path)
    G = nx.Graph()
    ...
    file_path = f'fb_tvshow-{model}-{int(h*100)}.pkl'
...
for h in bili:
    file_path = 'fb-pages-tvshow.edges'
    for model in ['IC', 'LT', 'SIS','ICI']:
        process_fb_food_dataset(file_path, model)
