...
def get_youtube_graph(
    path: str = "com-youtube.ungraph.txt.gz",
    *,
    sample_edges: int | None = None,
):
    return load_snap_youtube(
        path=path,
        directed=False,
        deduplicate=True,
...
def process_youtube_dataset(file_path, model='IC'):
    G = get_youtube_graph(path=file_path)
    ...
    file_out = f'youtube-{model}-{int(h*100)}.pkl'
...
for h in bili:
    file_path = 'com-youtube.ungraph.txt.gz'
    for model in ['IC', 'ICI', 'LT', 'SIS']:
        process_youtube_dataset(file_path, model)